...
Function ScrapWireArray(ObjectReference[] akWireArray, WorkshopScript akWorkshopRef)
	...
	int[] iFormIDs = new Int[0]
	while(i > 0)
		i -= 1
		
		if(akWireArray[i] != None)			
			ModTraceCustom(sPowerToolsLog, "     Deleting: " + akWireArray[i])
			iFormIDs.Add(akWireArray[i].GetFormID())
	...
	; 2.2.3 - Cdante added new safe method to clean grids
	F4SEManager.WSFWID_RemoveNodesFromPowerGrid(akWorkshopRef, iFormIDs)
EndFunction
...
Function RecreateWireArray(ObjectReference[] aWireArray, WorkshopScript akWorkshopRef)
	...
	; Before we start, let's remove any corrupt nodes to avoid crashes
	F4SEManager.WSFWID_CheckAndFixPowerGrid(akWorkshopRef, abFixAndScan = true)
	...
	while(i > 0)
		...
		if(aWireArray[i] != None)
			...
			int[] arriFormIDs = new Int[0]
			arriFormIDs.Add(aWireArray[i].GetFormID())
			...
			; 2.2.3 - Cdante added new safe method to clean grids
			F4SEManager.WSFWID_RemoveNodesFromPowerGrid(akWorkshopRef, arriFormIDs)
